---
title: Pig Science - Breeding - `r tools::toTitleCase(params$doctype)` 1
...
```{r empty-bp-diagram, echo=FALSE, hook_convert_odg=TRUE, fig_path="odg", out.width="80%"}
...
```{r solution-bp-diagram, echo=FALSE, hook_convert_odg=TRUE, fig_path="odg", out.width="80%"}
...
### Your Task
* Compute the number of daugthers $n$ that must be tested for a given sire such that $B$ is at least $`r b_p02`$. 
* How long does it take from the birth of a given sire to the time point where the predicted breeding value of the bull with a reliability of $B$ of at least $`r b_p02`$ is available? Here we assume that the first semen can be harvested from the bull at an age of $`r bull_age_p02`$ months and the average age at first calving is $`r afc_p02`$ months. The number of monthly milk controls to get a first lactation is assumed to be $`r nr_cont_per_lact`$. 
...
```{r sol_p02_b}
age_test_result <- bull_age_p02 + gest_len_p02 + afc_p02 + nr_cont_per_lact
```

* Age of the bull at first harvest of semen: $`r bull_age_p02`$ months
* Gestation length: $`r gest_len_p02`$ months
* Age at first calving: $`r afc_p02`$ months
* Number of monthly milk controls until lactation is completed: $`r nr_cont_per_lact`$
